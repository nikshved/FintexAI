...
def create_refresh_token(user: object):
    ...
    return jwt.encode(payload, settings.SECRET_KEY_REFRESH, algorithm=settings.ALGORITHM)


def create_verify_token(user: object):
    expire = datetime.now(timezone.utc) + timedelta(minutes=settings.VERIFY_TOKEN_EXPIRES)
    payload = {
        "user_id": user.id,
        "user_roles": user.roles,
        "type": "verify",
...
def decode_token(token: str, token_type: str):
    ...
    return jwt.decode(token, choice_token_type.get(token_type), algorithms=[settings.ALGORITHM])